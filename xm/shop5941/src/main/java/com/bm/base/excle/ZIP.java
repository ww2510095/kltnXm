...
//        File [] mfiles =unzip(zipFile, dest, passwd);
//        boolean a= true;
//        while (a){
//            a=deletefile(mfiles);
//        }
    ...
    static boolean deletefile(File[] mfiles){
        for (File mfile: mfiles){
            String[] kss =mfile.toString().split(".");
            if(kss[kss.length-1].equals("db")){
                if (mfile.isFile()) {
                    mfile.delete();
                    return true;
                }
    ...
    private static String buildDestinationZipFilePath(File srcFile,String destParam) {  
        if (Stringutil.isBlank(destParam)) {  
            if (srcFile.isDirectory()) {  
    ...
    public static void main(String[] args) throws ZipException { 
    	File[] li = unzip("C:\\a\\a.zip", "C:\\a\\", "");
    	for (File file : li) {
            Shop5941Application.out(file); ;
		}
         
//      try {  
//          File[] files = unzip("d:\\test\\汉字.zip", "aa");  
//          for (int i = 0; i < files.length; i++) {  
//              Application.out(files[i]);  
//          }  
//      } catch (ZipException e) {  
//          e.printStackTrace();  
//      }  
